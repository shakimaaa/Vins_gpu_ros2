...
void OctomapMapper::initMap(std::shared_ptr<rclcpp::Node> node) {
    ...
    RCLCPP_INFO(node_->get_logger(), "11Octomap parameters loaded: resolution=%.2f\n, prob_hit=%.2f\n, prob_miss=%.2f\n, occupancy_thresh=%.2f\n, localmap_thresh=%.2f\n, sliding_window=%d\n",
    ...
    RCLCPP_INFO(node_->get_logger(), "K fx=%.6f fy=%.6f cx=%.6f cy=%.6f | t_imu_cam=[%.6f %.6f %.6f]",
                camera_matrix_vec[0], camera_matrix_vec[4], camera_matrix_vec[2], camera_matrix_vec[5],
                t_imu_cam_vec[3], t_imu_cam_vec[7], t_imu_cam_vec[11]);
    ...
    m_octree_->setClampingThresMin(0.12); // default-ish
    m_octree_->setClampingThresMax(0.98);
    ...
    if (mp_.m_sub_cloud_from_xvins){
        ...
        auto qos = rclcpp::SensorDataQoS()
             .keep_last(1)
             .reliability(RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT)  // drop if slow
             .durability(RMW_QOS_POLICY_DURABILITY_VOLATILE);
        depth_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(
            "/camera/camera/depth/image_rect_raw", qos, std::bind(&OctomapMapper::depthCallback, this, std::placeholders::_1),
            cloud_sub_opt);
    }
    
    pose_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>(
        "/odometry", rclcpp::QoS(10).reliable(), std::bind(&OctomapMapper::poseCallback, this, std::placeholders::_1),
        pose_sub_opt);
...
void OctomapMapper::depthCallback(const sensor_msgs::msg::Image::SharedPtr msg){
    ...
    
    {
        ...
        const int VOX = std::max(1.0f, mp_.m_voxel_size);
        for (int v = 0; v < depth_image.rows; v += mp_.m_voxel_size) {  
            for (int u = 0; u < depth_image.cols; u += mp_.m_voxel_size) {
                ...
                if (depth > mp_.m_depth_pointcloudMaxX)
                {   
                    // add something to make it double voxelsize and skip
                     // --- thin clear rays to effective 2x stride (both u & v) ---
                    
                    // const int u_tile = u / VOX;
                    // const int v_tile = v / VOX;
                    // // keep only tiles where both indices are even -> doubles stride in each axis
                    // if ( (u_tile & 1) || (v_tile & 1) ) {
                    //     continue; // skip this pixel for clear rays
                    // } 
                    float X = mp_.m_depth_pointcloudMaxX;
                    float Y = (u - camera_matrix_.at<double>(0, 2)) * mp_.m_depth_pointcloudMaxX / camera_matrix_.at<double>(0, 0);
                    float Z = (v - camera_matrix_.at<double>(1, 2)) * mp_.m_depth_pointcloudMaxX / camera_matrix_.at<double>(1, 1);
                    ...
                    auto W = R * Epoint + t;
                    m_octree_-> insertRay(sensor_origin_, octomap::point3d(W.x(), W.y(), W.z()), mp_.m_depth_pointcloudMaxX); // free space
                    continue;
                }   
                // if(d==0) continue;
                ...
                // todo fix
                if(Wcloud.z() < mp_.resolution || Wcloud.z()>5){
                    continue;
                }
    ...
    const double dt = (node_->now() - current_time).seconds();  
    RCLCPP_INFO(node_->get_logger(), "elapsed = %.3f s", dt);
}
...
void OctomapMapper::poseCallback(const nav_msgs::msg::Odometry::SharedPtr msg){
     ...
     if (msg) {
        auto odometry = Eigen::Vector3f(msg->pose.pose.position.x, 
        ...
        // Eigen::Matrix3f R_imu_cam = T_imu_cam_.block<3,3>(0,0);
        // Eigen::Vector3f t_imu_cam = T_imu_cam_.block<3,1>(0,3);
        // auto p_cam = R_imu_cam * odometry + t_imu_cam;
        // sensor_origin_ = octomap::point3d(p_cam.x(),p_cam.y(),p_cam.z());
